    ...
    for year in range(2011,2021): # Year: 11 to 21
        try:
            for i in range(0,30): # Day in september: 0 to 30
                ...
                plt.savefig('D:/Documents/GitHub/auroral_indexes/Plots/' + station_name + '/Pannels/' + str(year) + '/Pannels - ' + date + '.png', dpi=300, bbox_inches='tight')
                plt.show()

                # The distribution part is no longer needed
                ...
                #     '[nT/s]','log(dB) [nT]')
                # plt.savefig('D:/Documents/GitHub/auroral_indexes/Plots/' + station_name + '/Distribution/' + str(year) + '/Horizontal distribution - ' + date + '.png', dpi=300, bbox_inches='tight')
                # plt.show()
                ...
                #             '[nT/s]','log(dB) [nT]')
                # plt.savefig('D:/Documents/GitHub/auroral_indexes/Plots/' + station_name + '/Distribution/' + str(year) + '/Vertical distribution - ' + date + '.png', dpi=300, bbox_inches='tight')
                # plt.show()
            iterations += 1
